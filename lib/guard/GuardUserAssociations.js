...
module.exports = (guard) => {
  ...
  /**
   * Check if a user has ANY of required permissions
   *
   * @param  {Array} permissions - Permissions to check for a user role
   *
   * @example
   * user.hasAnyOf(['view blogs','create blogs']);
   *
   * @returns {bool} - true if user has ANY of the given permissions
   */
  guard._UserModel.prototype.hasAnyOf = function (permissions) {
    return guard.userHasAnyPermissions(this, permissions);
  };
