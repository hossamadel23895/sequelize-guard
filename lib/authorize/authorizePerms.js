...
module.exports = (SequelizeGuard) => {
  ...
  * @private
  ...
  SequelizeGuard.prototype.userHasAnyPermissions = async function (
    user,
    permissions
  ) {
    const _guard = this;
    ...
    return firstTrue(
      roles.map((role) =>
        _guard
          ...
          .then((cache) => cache.getRolesWithPerms(_guard))
          .then((roles) => roles[role.id].Permissions)
          .then((perms) => this.resolvePermissions(perms, permissions))
      )
    );
  };
  ...
  */
  ...
  @private
  */
  SequelizeGuard.prototype.resolvePermissions = function (
    givenPermissions,
    wantedPermissions,
  ) {
    for (const wantedPermission of wantedPermissions) {
      if (this.resolvePermission(givenPermissions, wantedPermission)) {
        return true;
      }
    }
